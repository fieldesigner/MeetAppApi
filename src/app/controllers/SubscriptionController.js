...
class SubscriptionController {
  async store(req, res){
    ...
    if(meetup.user_id === req.userId){
      return res.status(401).json({ error: 'Você não pode ser inscrever em evento que vc esta organizando' });
    }
    ...
    if (meetSameDate) {
      return res.status(400).json({ error: "Você já esta inscrito para um evento no mesmo horário" });
    }
